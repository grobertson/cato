...
$(document).ready(function () {
    ...
    var d = '<div id="storm_run_dialog_parameters" class="floatleft task_launch_column ui-widget-content ui-tabs ui-corner-all">' +
                ...
                '	<br /><br />' +
                '	<input id="storm_run_dialog_compatibility" type="checkbox" /><label for="storm_run_dialog_compatibility">Skip Incompatible Resource Types?</label>' +
                '   <hr />' +
...
function getParamsFromJSON() {
    ...
    $.ajax({
        async: false,
        type: "POST",
        url: "uiMethods.asmx/wmGetEcotemplateStormParameterXML",
        ...
        success: function (msg) {
            if (msg.d == "") {
                ...
                storm_parameter_xml = $.parseXML(msg.d);
                $("#no_storm_msg").remove();
...
function getClouds() {
	...
	if (account_id) {
	    $.ajax({
	        ...
	        dataType: "json",
	        success: function (msg) {
	            var account = jQuery.parseJSON(msg.d);
...
function RunStorm() {
	...
	if (ecosystem_id.length == 36) {
		//we have a guid!  yay.
		var ecosys_url = "ecosystemEdit.aspx?tab=details&ecosystem_id=" + ecosystem_id;
...
function CreateEcosystem() {
    ...
    $.ajax({
        async: false,
        type: "POST",
        url: "uiMethods.asmx/wmCreateEcosystemByStorm",
        data: '{"sName":"' + packJSON(ecosystem_name) + '","sDescription":"' + ecosystem_desc + '","sEcotemplateID":"' + ecotemplate_id + '","sParameterXML":"' + parameter_xml + '","sCloudID":"' + cloud_id + '"}',
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (msg) {
            if (msg.d.length == 36) {
